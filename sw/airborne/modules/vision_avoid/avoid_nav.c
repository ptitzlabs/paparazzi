...
void angle_matcher(void) {
  ...
  float tmp = INFINITY;
  float tmp2 = INFINITY;
  /*Find the pivot distance location. It's simply the smallest distance between
  ...
  /*Swapping the rows and colum indexes for the pivot found previously to place
   ...
   * diagonal and finding the minimum distance values across rows/columns. This
   * sorts the distance values between potential matches in an ascending order.
   * A better way to do this would be to use a simplex method, but this works as
   * well.*/
  for (k = 0; k < n; k++) {
    /*Iterating across the diagonal*/
    tmp = D[i_index[k] * sn + j_index[k]];
    /*Check if current pivot is minumum across the column, swap the rows if a
    ...
    /*Check if current pivot is minimum across the row, swap the columns if a
  ...
  /*Calculate the number of matched points, source points to be dropped and new
  ...
  j = 0;
  k = 0;
  /*Update the ange values in the tracker slots for both source and destination.
   * Updating the destination is not
   * strictly necessary. Also add the new angles and drop the ones for which no
   * match was found.*/
  for (i = 0; i < m; i++) {
    if (i < n && i < n_matches) {
...
void request_obstacles(float* v, int* n) {
  ...
  /*Request obstacle data from the vision algorithm. First value in the array is
   * always returned as 0 due to some sorting errors, so instead it is replaced
...
void obstacle_update(float gamma, int i) {
  ...
  /*Check if the spread between the consecutive positions of the UAV is big
   * enough, i.e. if the parallax is good*/
    ...
    if (arena.obs[i].xy.x <= arena.lim && arena.obs[i].xy.y <= arena.lim) {
      ...
      for (m = -1; m < 3; m++) {
        for (n = -1; n < 3; n++) {
          ...
          if (loc_x_tmp >= 0 && loc_y_tmp >= 0 && loc_x_tmp < GRID_RES &&
              loc_y_tmp < GRID_RES) {
...
void nav_print_full_report(void) {
  ...
  for (i = 0; i < arena.sn; i++) {
    o = arena.obs[i];
    printf(
        "%i: original fix at %f,%f, latest fix at %f,%f, angle1: %f, angle2: "
